//! Drone Ollama — MAIN DELEGATE lean TUI (same UX as dronehive-tui).
//!
//! Layout (like Grok Build / dronehive-tui):
//!   chat transcript (user / drone / tool / system)
//!   tiny progress
//!   input bar
//!
//! Enter → `python -m drone app brain --command …`
//! Python streams `CHAT|role|text`; final `SEAL|{json}`.
...
#[derive(Parser, Debug)]
#[command(
    name = "drone-ollama-tui",
    about = "Drone Ollama MAIN DELEGATE chat console (lean TUI)"
)]
struct Args {
    #[arg(long)]
    root: Option<PathBuf>,
...
impl App {
    fn new(root: PathBuf, python: PathBuf) -> Self {
        let mut app = Self {
            root,
            python: python.clone(),
            input: String::new(),
            status: "ready · type command · Enter run · Esc quit".into(),
            progress: 0.0,
            busy: false,
            last_ok: None,
            tick: 0,
        ...
        app.push_chat(
            "system",
            format!(
                "Drone Ollama · MAIN DELEGATE · py={} · user → Ollama plan → swarm",
                python
                    .file_name()
                    .and_then(|s| s.to_str())
                    .unwrap_or("python")
        ...
        app.push_chat(
            "system",
            "same lean UX as DroneHive TUI · no canvas · chat + gauge + input",
        );
        app
    }
    ...
    fn submit(&mut self) {
        ...
        if goal.is_empty() {
            self.status = "type a command".into();
            return;
        }
        ...
        self.last_ok = None;
        self.progress = 0.08;
        self.status = format!("delegating… {goal}");
        self.push_chat("user", goal.clone());
        self.push_chat("system", "Ollama commander → drones…");
        ...
        thread::spawn(move || {
            run_brain_job_stream(&py, &root, &g, tx);
        });
    }
    ...
    fn poll_job(&mut self) {
        ...
        let mut pending: Vec<JobMsg> = Vec::new();
        loop {
            match rx.try_recv() {
            ...
            match msg {
                ...
                JobMsg::Done {
                    ...
                    if let Some(e) = err {
                        ...
                        self.status =
                            format!("{status} · action={action} · {seal_name}");
                        self.push_chat(
                            "system",
                            format!("done · {status} · action={action} · {seal_name}"),
                        );
                        if ok {
                            self.input.clear();
                        }
...
fn write_last_error(root: &Path, msg: &str) {
    ...
    let _ = std::fs::write(root.join("out").join("TUI_OLLAMA_LAST_ERROR.txt"), msg);
}

fn run_brain_job_stream(
    python: &Path,
    root: &Path,
    command: &str,
    ...
    if let Some(v) = seal_json.or_else(|| parse_json_blob(&stdout_tail)) {
            ...
            .or_else(|| {
                v.get("pipeline")
                    ...
                    .and_then(|a| a.iter().find_map(|x| {
                        x.as_str().and_then(|s| s.strip_prefix("delegate:"))
                    }))
            })
            .unwrap_or("?")
            .to_string();
        ...
        let _ = tx.send(JobMsg::Done {
            ...
            err: if ok {
                ...
                Some(format!(
                    "{status} · see out\\TUI_OLLAMA_LAST_ERROR.txt"
                ))
            },
        });
    } else {
...
fn main() {
    ...
    let app = App::new(root.clone(), python);
